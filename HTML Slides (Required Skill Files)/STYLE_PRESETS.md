...
Curated visual styles for Brand Slides. Each preset is inspired by real design references — no generic "AI slop" aesthetics. **Abstract shapes only — no illustrations.**

> **Have a brand or design reference instead of picking a curated look?** Use [DESIGN_EXTRACTION.md](DESIGN_EXTRACTION.md) instead. It produces the exact same `:root` token block as these presets below — just derived from the user's own brand (screenshot, brand book, hex codes, or logo) rather than a curated style. Phase 2 in `SKILL.md` decides which path to use.
