---
name: brand-slides
description: Create stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files — and automatically match an existing brand or design reference (screenshot, URL, brand book, hex codes, or logo) instead of picking from generic presets. Use when the user wants to build a presentation, convert a PPT/PPTX to web, create slides for a talk/pitch/course, or specifically wants a deck that matches their own brand identity. Falls back to visual style discovery (curated previews) when no design reference exists.
---

# Brand Slides

Create zero-dependency, animation-rich HTML presentations that run entirely in the browser — styled either by curated visual discovery, or by extracting real design tokens from a brand reference the user already has.

This skill is a fork of the `frontend-slides` workflow with one structural change: **Phase 2 now has two paths.** If the user has something to match (a screenshot, a brand book, hex codes, a logo), the skill extracts design tokens directly from it instead of generating abstract style previews. If they don't, the original curated-preview discovery flow runs unchanged.

## Core Principles

1. **Zero Dependencies** — Single HTML files with inline CSS/JS. No npm, no build tools.
2. **Match Real Input Over Guessing** — If a design reference exists, derive the style from it. Never substitute a "close enough" generic look when the real thing was provided.
3. **Show, Don't Tell (when there's nothing to match)** — When no reference exists, generate visual previews, not abstract choices. People discover what they want by seeing it.
4. **Distinctive Design** — No generic "AI slop." Every presentation must feel custom-crafted, whether brand-matched or freshly discovered.
5. **Progressive Disclosure** — Read lightweight style indexes first. Load full design recipes only after a direction is chosen or confirmed.
...
You tend to converge toward generic, "on distribution" outputs. In frontend design, this creates what users call the "AI slop" aesthetic. Avoid this: make creative, distinctive frontends that surprise and delight — and when a real brand reference exists, avoid it doubly by actually using that brand's own choices instead of a generic substitute.

Focus on:

- Typography: Choose fonts that are beautiful, unique, and interesting. Avoid generic fonts like Arial and Inter; opt instead for distinctive choices that elevate the frontend's aesthetics — or the reference's own fonts when one exists.
- Color & Theme: Commit to a cohesive aesthetic. Use CSS variables for consistency. Dominant colors with sharp accents outperform timid, evenly-distributed palettes. Draw from IDE themes and cultural aesthetics for inspiration when there's no brand to match.
...
- Overused font families (Inter, Roboto, Arial, system fonts) — unless a brand reference itself genuinely uses one
...
These invariants apply to EVERY slide in EVERY presentation, regardless of which style path produced it:
...
| **High density / reading-first** | Reports, handouts, async review, training/course material, detailed internal docs | More self-contained slides, structured grids/tables/annotations, 4-8 bullets or 4-6 cards when readable, tighter but still intentional spacing |
...
**Question 5 — Design Source** (header: "Design Source"):
Do you have a brand or design reference for this deck to match? Options:

- "Yes — I'll share a screenshot, URL, brand book, hex codes, or logo" → Phase 2 runs the **Design Input Path**
- "No — show me curated style options" → Phase 2 runs the **Curated Style Path** (original visual discovery)

This decides which of the two Phase 2 paths runs below. It is asked once per deck — if the user is building a series (e.g., one course with many lecture decks), Question 5 only needs a real answer on the first deck; later decks in the same series can default to "match the design we already confirmed" unless the user says otherwise.
...
**Logo in previews:** If a usable logo was identified, embed it (base64) into the Phase 2 preview(s) — the user sees their brand styled in context, not as an abstract swatch.

---

## Phase 2: Style Discovery

Phase 1, Question 5 decides which path runs here. **Do not run both** — pick one based on the user's answer.

### Path A — Design Input (user has something to match)

Read **[DESIGN_EXTRACTION.md](DESIGN_EXTRACTION.md)** in full and follow it exactly. In short:

1. Collect whatever design reference the user has (screenshot, URL, brand book/PDF, plain-text hex codes + font names, or a logo file).
2. Extract design tokens from it — colors, fonts, logo, tone — following the extraction-priority order in `DESIGN_EXTRACTION.md` (stated → inferred → asked). Never silently substitute a generic default.
3. Run the Design Fidelity Checklist in that file before showing anything to the user.
4. Render exactly **one** preview slide using the extracted tokens, saved to `.frontend-slides/slide-previews/design-input.html`, and open it automatically.
5. Ask (header: "Design Match"): _"Does this match your brand?"_ Options: **Looks right** / **Adjust a color or font** / **Show me curated style options instead**
6. On "Adjust," refine and re-render the same single preview — don't generate new alternatives. On "Show me curated options instead," fall through to Path B.

Once confirmed, the extracted token block **is** the style for this deck — carry it directly into Phase 3 as if it were a chosen preset.

### Path B — Curated Style Discovery (no reference to match)

**This is the "show, don't tell" phase.** Most people can't articulate design preferences in words.

#### Step 2.B0: Generate 3 Style Previews Directly
...
Read [STYLE_PRESETS.md](STYLE_PRESETS.md) for safe preset candidates. If `bold-template-pack/selection-index.json` exists, read that compact index too, but do not read any `design.md` files yet.
...
- Generate 3 previews by default: 1 safe preset from `STYLE_PRESETS.md`, at least 1 bold template from `bold-template-pack/selection-index.json` (if that pack is present), and 1 wildcard.
...
Generate the full presentation using content from Phase 1 (text, or text + curated images) and style from Phase 2 — regardless of which Phase 2 path produced it.
...
If the user's stated needs are mixed, choose the closer of the two modes instead of inventing a middle option: live audience persuasion defaults low-density; async circulation, training material, or detailed review defaults high-density.
...
**If Phase 2 ran the Design Input Path:** treat the confirmed Design Token Block from `DESIGN_EXTRACTION.md` as the design recipe. Preserve its colors, fonts, logo treatment, and tone exactly as confirmed — do not drift toward a preset or introduce colors/fonts that weren't part of the extracted set.

**If Phase 2 ran the Curated Style Path and the user selected a bold template** from `bold-template-pack`, read that one template's full `design.md` before generating. Do not read the other bold templates. Treat `design.md` as the design recipe:
...
- Keep the output as a single self-contained Brand Slides HTML file.
...
**If Phase 2 ran the Curated Style Path and the user selected a self-generated custom wildcard,** treat that preview's CSS and layout as the design recipe:
...
- Use fonts from Fontshare or Google Fonts — never system fonts (unless the Design Input Path confirmed the brand genuinely uses one)
...
3. **Style selection** — Proceed to Phase 2 for style discovery (either path)
...
3. **Summarize** — Tell the user:
   - File location, style name (or "matched to [brand]" if Design Input Path was used), slide count
...
| --------------------------------------------------- | --------------------------------------------------------------------- | -------------------------- |
| [DESIGN_EXTRACTION.md](DESIGN_EXTRACTION.md)       | How to extract design tokens from a brand/design reference            | Phase 2, Design Input Path |
| [STYLE_PRESETS.md](STYLE_PRESETS.md)               | 12 curated visual presets with colors, fonts, and signature elements | Phase 2, Curated Style Path |
| `bold-template-pack/selection-index.json` (optional) | Compact bold template metadata for candidate selection | Phase 2, Curated Style Path |
| `bold-template-pack/templates/*/preview.md` (optional) | Lightweight style cards for shortlisted bold title previews | Phase 2, after shortlisting |
| `bold-template-pack/templates/*/design.md` (optional) | Detailed design-system docs for the selected bold template only | Phase 3, after user selection |
...
`bold-template-pack/` is optional — the skill degrades gracefully to `STYLE_PRESETS.md` alone if that folder isn't present.
